...
void bb_callback(const darknet_ros_msgs::BoundingBoxes::ConstPtr& msg){
    ...
    std::for_each(v.begin(), v.end(), [](const darknet_ros_msgs::BoundingBox& x){
        ...
        z_c = ((int) x.ymin + (int) x.ymax)/2;
        dist = depthImage.at<unsigned short>(z_c, x_c);

        objectPosition.Class = x.Class;
        objectPosition.x = (dist*(x_c - cx)/fx - 10); //35 is the distance between rgb camera module and center of realsene
        objectPosition.y = dist  ;
        objectPosition.z = -dist*(z_c - cy)/fy+110;
